...
static void RtlInitUnicodeString(PUNICODE_STRING dest, const wchar_t* src) {
    if (!dest) return;
    if (src) {
        size_t len = wcslen(src);
        dest->Length = (USHORT)(len * sizeof(wchar_t));
        dest->MaximumLength = (USHORT)((len + 1) * sizeof(wchar_t));
...
bool RegisterAndStartService(const wchar_t* driver_name, const wchar_t* driver_path) {
    ...
    PFN_RtlAdjustPrivilege RtlAdjustPrivilege =
        (PFN_RtlAdjustPrivilege)GetProcAddress(ntdll, "RtlAdjustPrivilege");
    PFN_NtLoadDriver NtLoadDriver =
        (PFN_NtLoadDriver)GetProcAddress(ntdll, "NtLoadDriver");
...
bool StopAndRemoveService(const wchar_t* driver_name) {
    ...
    PFN_NtUnloadDriver NtUnloadDriver =
        (PFN_NtUnloadDriver)GetProcAddress(ntdll, "NtUnloadDriver");
    if (!NtUnloadDriver) {
